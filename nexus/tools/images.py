...
class ImagesTool(BaseTool):
    ...
    @staticmethod
    def _preview(action: str, model: str, prompt: str, input_count: int) -> str:
        return "\n".join(
            [
    ...
    async def run(self, args: dict[str, Any]) -> ToolResult:
        ...
        if not args.get("confirmed"):
            return ToolResult(
                ok=False,
                content=self._preview(action, model, prompt, len(resolved_inputs)),
        ...
        try:
            if action == "generate":
                data = self.client.generate(prompt=prompt, model=model)
            else:
                data = self.client.edit(prompt=prompt, input_paths=resolved_inputs, model=model)
